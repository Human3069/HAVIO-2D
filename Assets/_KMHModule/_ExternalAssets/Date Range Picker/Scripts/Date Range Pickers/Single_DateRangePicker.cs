...
public class Single_DateRangePicker : DateRangePicker
{
    ...
    [SerializeField]
    private UITweenManager UITweenManager;
    [SerializeField]
    private bool m_ShowDaysInOtherMonths = false;
    [SerializeField]
    private bool m_CloseOnLastSelection = false;
    ...
    [SerializeField]
    protected float toolbarHeight = 80f;
    [SerializeField]
    protected float fullHeight = 800f;

    private RectTransform _rectT;

    [Space(10)]
    public UnityEvent<List<DateTime>> OnSelectionEnd;
    public UnityEvent<bool> OnValueChangedCalendarToggle;
    ...
    {
        _rectT = this.transform as RectTransform;
        UpdateToggleCalendars();
    ...
    public void OnPointerDown(CalenderButton chosenCalenderButton, DateTime chosenDate, Calender calender)
    {
        ...
        if (m_StartDate != null && m_EndDate == null)
        {
            ...
            DateTime currentDate = m_StartDate.Value;
            while (currentDate <= m_EndDate.Value)
            {
                dateTimeList.Add(currentDate);
                currentDate = currentDate.AddDays(1); // 1일 증가
            }

            OnSelectionEnd.Invoke(dateTimeList);
            return;
        }
    }
    ...
    public void OnClick_ToggleCalenders()
    {
        ...
        OnValueChangedCalendarToggle.Invoke(m_Calender.gameObject.activeInHierarchy);
    }

    protected void UpdateToggleCalendars()
    {
        bool isOn = m_Calender.gameObject.activeInHierarchy;
        if (isOn == true)
        {
            _rectT.sizeDelta = new Vector2(_rectT.sizeDelta.x, fullHeight);
        }
        else
        {
            _rectT.sizeDelta = new Vector2(_rectT.sizeDelta.x, toolbarHeight);
        }
    }
}
